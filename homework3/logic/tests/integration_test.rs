...
use logic::logic::config::ConfigVariant;
use logic::logic::error::OperationError;
use logic::logic::run;
...
fn test_df_generate_without_parameters_console() {
    let args = ["df", "generate", "console"].iter().map(|s| s.to_string());
...
fn test_rsa_encrypt_console() {
    let args = [
        ...
        "encrypt",
        "console",
        "Test RSA target string!",
        "9683922000451682283955009414215846271",
        "503389953040597954843496152539898795547523683",
    ]
    .iter()
    .map(|s| s.to_string());
...
fn test_rsa_generate_console() {
    let args = ["rsa", "generate", "console"].iter().map(|s| s.to_string());
